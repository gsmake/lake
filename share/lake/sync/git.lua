...
function module:sync_remote(remote)
    ...
    local workdir = filepath.join(self.lake.GSMAKE_HOME_REPO,"git",self.package.Name)
    ...
    local sourceTarget = filepath.join(workdir,self.package.Version)
    ...
    exec:dir(workdir)
    exec:start("clone",target,self.package.Version)

    if exec:wait() ~= 0 then
        error(string.format("clone git repo from %s -- failed",target))
    end

    local version = self.package.Version
    ...
    self.db:save_sync(self.package.Name,self.package.Version,remote,target,"git")

    self.db:save_source(self.package.Name,self.package.Version,target,sourceTarget)

end

function module.ctor(lake,package)
