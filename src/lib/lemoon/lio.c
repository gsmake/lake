...
static size_t __io_hash(handler_t handle, size_t buckets){
...
LEMOON_PRIVATE lfile* lfile_new(lua_State *L, lio * io, size_t len, const char * tname, handler_t fd, const luaL_Reg * funcs, lua_CFunction closef)
{
...
LEMOON_PRIVATE lfile* lfile_search(lio * io, handler_t fd)
{
    size_t hashcode = __io_hash(fd, io->buckets);
