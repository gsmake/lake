...
static luaL_Reg lemoon_funcs[] = {{"timer", ltimer_new},
                                  {"io", lio_new},
...
LEMOON_API int lemoon_error(lua_State *L, const char *file, int lines,
                            const char *msg, ...) {
    ...
    if (msg) {
        ...
        lua_pushfstring(L, "lemoon panic :\n\tfile:%s\n\tlines:%d", file,
                        lines);
        luaL_where(L,1);
    }
...
LEMOON_API void lemoon_pusherror(lua_State *L, const char *file, int lines,
                                 const char *msg, ...) {
  luaL_where(L, 1);
  if (msg) {
    va_list argp;
...
LEMOON_API int lemoon_sysmerror(lua_State *L, int errcode, const char *file,
                                int lines, const char *msg, ...) {
  luaL_where(L, 1);
  lua_pushfstring(L, "lemoon runtime exception:\n\tfile:%s\n\tlines:%d\n\tmsg:",
                  file, lines);
...
LEMOON_API void lemoon_pushsysmerror(lua_State *L, int errcode,
...
LEMOON_API void *lemoon_newclass(lua_State *L, const char *name,
                                 size_t classize, const luaL_Reg *funcs,
